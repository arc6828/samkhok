...
$footer_feed = get_samkhok_medium_feed();
$search_autocomplete_data = [];
if (isset($footer_feed->channel->item) && is_array($footer_feed->channel->item)) {
    foreach ($footer_feed->channel->item as $s_item) {
        $clean_text = preg_replace('/[^ก-๛0-9\s]+/u', '', strip_tags($s_item->contentEncoded ?? ''));
        $search_autocomplete_data[] = [
            "title" => $s_item->title ?? '',
            "content" => $clean_text,
            "link" => $s_item->link ?? '#'
...
<script>
    (function() {
        ...
        if (searchInput) {
            const searchData = <?= json_encode($search_autocomplete_data, JSON_UNESCAPED_UNICODE) ?>;
